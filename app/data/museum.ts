...
export const museum: Wing[] = [
  {
    ...
    exhibits: [
      ...
      { name: 'wget', how: 'wget https://api.github.com/zen', blurb: 'curl prints; wget keeps. The response lands as a real file in the filesystem — cat it, vim it, grep it. Clipped at 64K, because your quota lives in localStorage. killall, its neighbor, ends processes by name instead of pid.' }
    ]
  },
  {
  ...
  },
  {
    ...
    exhibits: [
      ...
      { name: 'the pair terminal', how: 'pair, then a friend types pair join <code>', blurb: 'Share your live shell. Hosting mints a four-letter code; whoever joins watches your transcript stream in real time, and pair allow hands them the keyboard — their commands run on your shell, output flowing back to everyone. The relay referees the room; kill pair-share.sh (or pair stop) ends the broadcast.' }
    ]
  },
  {
    ...
    exhibits: [
      ...
      { name: 'a citizen of your phone', how: 'install it, then share a link to it', blurb: 'Installed, it behaves like it belongs: the OS share sheet can send any link or text straight into lvOS as a sticky note in ~/stickies, and the app icon wears the unread-mail count — the same single read-state the desktop badge uses — clearing as you read.' },
      { name: 'the frame budget', how: 'drag a window over the life wallpaper', blurb: 'A whole pass on where the milliseconds go: windows drag on the compositor instead of re-blurring their glass per move, the hero crosshair stopped forcing a reflow per mousemove, the pixel-world minimap blits a cached board instead of rescanning 16k cells a frame, and settled terminal lines are never re-diffed. Same look, calmer CPU.' },
