...
const SIMPLE_APPS: Record<string, ReturnType<typeof defineAsyncComponent>> = {
  ...
  timemachine: defineAsyncComponent(() => import('~/components/desktop/DesktopTimeMachine.vue'))
}
...
// ---- right-click context menu (+ touch long-press) ----
const {
  contextMenu, openContextMenu, closeContextMenu,
  onDesktopTouchStart, onDesktopTouchMove, onDesktopTouchEnd
} = useDesktopContextMenu()
const titleMenu = reactive({ open: false, x: 0, y: 0, winId: '' })

// a tap anywhere dismisses both menus
const closeMenus = () => {
  closeContextMenu()
