...
- **Works on a phone** — the whole thing folds down to 375px: a bottom tab bar for the everyday pages, a bottom-sheet terminal with a quick-key row (so the games are thumb-playable), and an lvOS desktop where windows open maximized so their controls stay reachable, the taskbar scrolls to keep its tray, a long-press opens the desktop context menu (and a file's row menu), and Snake grows a d-pad. Minesweeper flags with a flag-mode toggle and shrinks its board to fit, every app's buttons grow to thumb size, and wide tables and the /life controls wrap or scroll instead of clipping.
