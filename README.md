...
- **A persistent virtual filesystem** — `ls`, `cd` (incl. `cd -`, `pushd`/`popd`), `pwd`, `tree`, `cat`, `mkdir`, `touch`, `rm`, `cp`, `mv` — with `*` wildcards, and it survives across visits. Edit files with the real **`nano`** and modal **`vim`**/`vi`, and `wget <url>` downloads a real response into it (curl prints; wget keeps). Anything you `rm` lands in the lvOS **recycle bin**, restorable from the desktop.
...
- **One process table** — `ps`/`kill`/`top` see everything: terminal effects, **open lvOS windows**, and the running game or editor, with the same pids the desktop task manager shows. `kill` really closes windows, ends games (`killall <name>` does it by name) — and `kill 7` terminates your own shell.
...
- **Multiplayer** (when the relay is on) — `chat` joins the `#lounge`; `pong online`, `wpm race` and online chess match you against a live visitor, refereed by a server-authoritative relay (they fall back to solo play otherwise); and **`pair`** shares your live shell — it mints a four-letter code, `pair join <code>` streams your transcript to whoever holds it, and `pair allow` hands them the keyboard so their commands run on your shell. `mail` shares the lvOS inbox; `bug` opens a prefilled GitHub issue; and `jq` runs a real subset of jq over the site's own JSON (`resume`, `profile`, `projects`, or piped in from `curl`).
...
- Dark/light theme throughout, reduced-motion respected everywhere, and filterable projects with detail pages.
- **Works on a phone** — the whole thing folds down to 320px: a bottom tab bar for the everyday pages, a bottom-sheet terminal (which no longer jumps sideways on open) with a quick-key row and grid games that scroll instead of wrapping, and an lvOS desktop where windows open maximized so their controls stay reachable, the taskbar collapses its tray behind a ⋯, a long-press opens the desktop context menu (and a file's row menu), and Snake grows a d-pad. Minesweeper flags with a flag-mode toggle and shrinks its board to fit, the status bar, filter chips and pixel-world swatches all grow to thumb size, every app's buttons follow, and wide tables and the /life controls wrap or scroll instead of clipping. `/keyboard` drops its wasted key column, the pixel-world minimap stays inside its frame, and the lvOS Settings options wrap instead of clipping off the window edge. New apps stay thumb-friendly too: Asteroids gets an on-screen rotate/thrust/fire pad on touch, and the Pet shop's buttons and the whiteboard's four action buttons (undo · replay · save · clear) wrap rather than overflow at 320px. This round the snake board itself shrinks so the whole arena fits a phone, tmux splits stack vertically instead of clipping half a prompt into a 186px column, the contact wizard's titlebar ellipsizes like the terminal's, the mobile menu's action row and the taskbar clock/tray grew to real tap sizes — and the command palette's first open finally focuses its input (typing and <kbd>Esc</kbd> were dead until a second open, on every device).
