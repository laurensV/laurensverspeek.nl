...
- **Multiplayer** (when the relay is on) — `chat` joins the `#lounge`, and `pong online`, `wpm race` and online chess match you against a live visitor, refereed by a server-authoritative relay (they fall back to solo play otherwise). `mail` shares the lvOS inbox; `bug` opens a prefilled GitHub issue; and `jq` runs a real subset of jq over the site's own JSON (`resume`, `profile`, `projects`, or piped in from `curl`).
